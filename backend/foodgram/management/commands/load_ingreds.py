...
class Command(BaseCommand):
    ...
    help = (
        ...
        'ВАЖНО: '
        'Каждый файл должен включать в себя только название продукта и единицу измерения для него. '
        'При загрузке из csv, в качестве разделителя, должна использоваться запятая. '
        'JSON должен включать в себя только поля "name" и "measurement_unit". '
        'При выполнении команды нужно указать путь до файла относительно КОРНЯ ПРОЕКТА.'
    )
